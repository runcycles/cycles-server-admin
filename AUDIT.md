# Complete Budget Governance v0.1.25.38.1 — Admin Server Audit

**Server version:** 0.1.25.38.1 (2026-04-23 — same-release hygiene sweep: actor-type symmetry in `emitBulkFundEvent`, payload-map `ArgumentCaptor` assertions on bulk emit tests, README `v0.1.25.x` catalog backfill .32–.38; no behavior / API / spec change; v0.1.25.38 baseline: spec v0.1.25.32 bulk-action event parity with per-row Events on `bulkActionBudgets` + `bulkActionTenants`)
