...
| `webhook.secret.encryption-key` | `WEBHOOK_SECRET_ENCRYPTION_KEY` | (unset; startup fails) | Base64 AES-256 key used to encrypt webhook signing secrets at rest. Must match the value on `cycles-server-events`. Existing non-`enc:` values remain readable for migration; all new or rotated writes are encrypted. Production Compose requires the key. |
| `webhook.secret.allow-plaintext` | `WEBHOOK_SECRET_ALLOW_PLAINTEXT` | `false` | Explicit local/development compatibility opt-out. When true with an empty key, startup warns prominently and signing secrets remain unencrypted in Redis. Never enable in production. |
